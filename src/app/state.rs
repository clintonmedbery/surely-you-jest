...
impl App {
    ...
    pub fn go_back(&mut self) {
        if self.view == AppView::TestRunning
            && self.running_individual_test
            && !self.individual_tests.is_empty()
        {
    ...
    pub fn scroll_down(&mut self, amount: usize) {
        if self.view == AppView::TestRunning {
            ...
            self.terminal_scroll =
                (self.terminal_scroll + amount).min(line_count.saturating_sub(1));
        }
    }
    ...
    pub fn parse_test_results(&mut self) {
        ...
        for line in self.test_run_output.lines() {
                ...
                if name_parts.len() > 1 {
                    ...
                    if let Some(duration_idx) = current_test_name.find("(") {
                        if let Some(end_idx) = current_test_name.find(")") {
                            ...
                            current_test_name =
                                current_test_name[0..duration_idx].trim().to_string();
                        }
                    }
                }
            ...
            } else if !current_test_name.is_empty()
                && line.contains("Error:")
                && !current_test_passed
            {
                // Found error details
    ...
    pub fn run_individual_test(&mut self) -> io::Result<()> {
        if self.individual_tests.is_empty()
            || self.selected_test_index >= self.individual_tests.len()
        {
            return Ok(());
        }
        ...
        self.test_run_output = format!(
            ...
            test_name, test_file, test_file, test_name_pattern
        );
    ...
    pub fn render(&mut self, frame: &mut Frame) {
        use crate::widgets::{
            HeaderWidget, HelpBarWidget, SpinnerWidget, TestDetailWidget, TestListWidget,
            TestResultsWidget, TestTerminalWidget,
        };
        ...
        let (title, subtitle) = match self.view {
            ...
            AppView::TestDetail => {
                ...
                ("Test File".to_string(), format!("{}", test_name))
            }
            AppView::TestRunning => {
                let test_name = if !self.tests.is_empty() {
                    &self.tests[self.selected_index]
        ...
        frame.render_widget(HeaderWidget { title, subtitle }, chunks[0]);

        // Render appropriate content based on the current view
        match self.view {
            AppView::TestList => {
                let widget =
                    TestListWidget::new(&self.tests, self.selected_index, self.scroll_offset);
            ...
            AppView::TestResults => {
                let widget =
                    TestResultsWidget::new(&self.individual_tests, self.selected_test_index);
                frame.render_widget(widget, chunks[1]);
            }
        }
